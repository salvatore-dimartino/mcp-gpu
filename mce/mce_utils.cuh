...
namespace graph
{
  ...
  template <typename T, uint CPARTSIZE = 32>
  __device__ __forceinline__ uint64 warp_sorted_count(
      const T *const A, const size_t aSz, T *B,
      T bSz, T j, T num_divs_local, T *degree)
  {
    const int warpIdx = threadIdx.x / CPARTSIZE; // which warp in thread block
    const int laneIdx = threadIdx.x % CPARTSIZE; // which thread in warp
    // cover entirety of A with warp
    for (T i = laneIdx; i < aSz; i += CPARTSIZE)
    {
      ...
      if (found)
      {
        atomicInc(&degree[j], 1);
        atomicInc(&degree[i], 1);
      }
    }
    return 0;
  }
  ...
  /*
  * Builds in a blob of memory a bitset representing the clique where each index 
  * of the bit to one represent the i-th neighboruhhod of the candidate vertex
  */
  template <typename T, uint CPARTSIZE = 32>
      ...
      T bSz, T j, T num_divs_local, T *encode, T base)
  {
    ...
    for (T i = laneIdx; i < aSz; i += CPARTSIZE)
    {
      ...
      if (found)
      {
        ...
        // if in the second part of the neighbourhood
        if (j >= base)
        {
...
/*
* Compares the maximum intersection pivot found from all warps, and get the maximum
*/
template <typename T, uint BLOCK_DIM_X, uint CPARTSIZE>
    ...
    SHARED_HANDLE<T, BLOCK_DIM_X, CPARTSIZE> &sh)
{
  for (T j = threadIdx.x; j < sh.num_divs_local; j += blockDim.x)
  {
    ...
    sh.pl[j] = ~(sh.encode[(sh.level_pivot)*sh.num_divs_local + j]) & m; // Removes Neighbours of pivot
    sh.cl[j] = m;
    sh.xl[j] = 0;
  }
  __syncthreads();
...
__device__ __forceinline__ void wait_for_donor_dyn(
    cuda::atomic<uint32_t, cuda::thread_scope_device> &work_ready, uint32_t &shared_state, uint32_t current_queue_size,
    queue_callee(queue, tickets, head, tail))
{
  uint32_t ns = 8;
  do
  {
    ...
    else if (queue_full(queue, tickets, head, tail, current_queue_size))
    {
      shared_state = 100;
      break;
    }
    ...
    LOCAL_HANDLE<T> &lh, SHARED_HANDLE<T, BLOCK_DIM_X, CPARTSIZE> &sh, const T &lim)
{
  ...
  {
    uint64 warpCount = 0;
    // Count in parallel the number of neighbour 
    for (T k = lh.lx; k < sh.num_divs_local; k += CPARTSIZE)
    {
    ...
    // the first thread of each lane contains the total number of neighbour
    if (lh.lx == 0 && sh.maxCount[lh.wx] == lim + 1)
    {
    ...
    const T &Xx_sz, const T &lim)
{
  for (T j = lh.wx; j < Xx_sz; j += lh.numPartitions)
  {
    ...
    // The count of neighbour of a pivot chosen from X is greater than the one chosen from P
    else if (lh.lx == 0 && sh.maxCount[lh.wx] < warpCount)
    {
      sh.maxCount[lh.wx] = warpCount;
      sh.maxIndex[lh.wx] = cur;
    ...
    GLOBAL_HANDLE<T> &gh, SHARED_HANDLE<T, BLOCK_DIM_X, CPARTSIZE> &sh)
{
  if (threadIdx.x == 0)
  {
    sh.src = gh.gsplit.rowInd[sh.i]; // i is the index of the i'th vertex, 
    sh.srcStart = gh.gsplit.splitPtr[sh.src]; 
    sh.srcLen = gh.gsplit.rowPtr[sh.src + 1] - sh.srcStart; // length of the row neighbourhood
    sh.src2 = gh.gsplit.colInd[sh.i];
    sh.src2Start = gh.gsplit.splitPtr[sh.src2];
    sh.src2Len = gh.gsplit.rowPtr[sh.src2 + 1] - sh.src2Start;
    sh.usrcStart = gh.gsplit.rowPtr[sh.src];
